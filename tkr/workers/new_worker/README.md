...
*TODO*: You will implement the functionality of the [`old-worker`](../old_worker/README.md) using Guppy instead of pytket.
The below will give you an introduction to the general worker setup.
After reading this you can proceede to [`impl.py`](./tkr_new_worker_impl/impl.py) to implement the new functionality.
For this scenario some of the functionality is already set up for you, which would not be the case for freshly generated worker.
...
└── pyproject.toml
```
The `tkr_new_worker_impl` directory contains the worker tasks in `impl.py` which are used during runtime of a workflow.
...
The source code is packaged as `trk-new_worker-impl`.
...
They are encapsulated in a separate package `tkr-new_worker` to avoid unnecessary dependencies during construction of the worker.
...
1. Using the `tkr-new_worker` package.
